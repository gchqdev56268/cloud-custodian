...
class AutoTagRole(EventAction):
    """Tag a resource with the role who created/modified it.
    ...
    def validate(self):
        if self.manager.data.get('mode', {}).get('type') != 'cloudtrail':
            raise PolicyValidationError(
                "Auto tag role requires an event %s" % (self.manager.data,))
    ...
    def register_resource(cls, registry, resource_class):
        if 'auto-tag-role' in resource_class.action_registry:
            return
        if resource_class.action_registry.get('tag'):
            resource_class.action_registry.register('auto-tag-role', AutoTagRole)


resources.subscribe(AutoTagRole.register_resource)
